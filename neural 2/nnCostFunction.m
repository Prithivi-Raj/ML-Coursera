...
J=((-1/m)*sum(sum((b.*log(h))+((1-b).*log(1-h)))));
...
Theta2_grad= (1/m)*(s3'*a2);
Theta1_grad=(1/m)*(s2'*a1);
...
regJ= (lambda/(2*m))*(sum(sum(v.^2))+sum(sum(f.^2)));
...
Theta2_grad=Theta2_gradreg+Theta2_grad;
